...
train_ds = KMNIST_20x20_Z(train_imgs, train_labels, n=5000, resolution=28)
test_ds  = KMNIST_20x20_Z(test_imgs,  test_labels,  n=5000, resolution=28)
...
def train(f, params, train_dl, test_dl):
    ...
    for i in range(num_digits):
      zero_vec = tc.zeros((28,1), dtype = tc.float32)
      zero_vec[i] = 1
...
def build_digit_tensor_class(m=28**2, num_digits=10):
    digit_tensor_class = tc.zeros((num_digits, m), dtype=tc.float32)
    for i in range(num_digits):
        zero_vec = tc.zeros((28, 1), dtype=tc.float32)
        zero_vec[i] = 1.0
...
def plot_test_predictions(model, test_dl, device="cpu", n_show=16):
    model.eval()
    digit_tensor_class = build_digit_tensor_class(m=28*28, num_digits=10).to(device)
